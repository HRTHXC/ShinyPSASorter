...
for(i in 1:fatherUniques){
  ...
  for(j in 1:motherUniques){
    ...
    planted[i] <- planted + plantedDebug[currentIndex]
    psaDeath[i] <- psaDeath + psaDeathDebug[currentIndex]
    lifeRate <- (100 - ((psaDeath / planted) * 100))
    lifeRate[i] <- format(round(lifeRate, 2), nsmall = 2)
  ...
  fatherNameMeanTable <- data.frame(planted, psaDeath, lifeRate)
}
print(dim(fatherNameMeanTable))
print(as.data.frame.matrix(fatherNameMeanTable[1:fatherUniques, ]))
