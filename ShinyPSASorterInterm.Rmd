...
for (i in 1:motherUniques){
  for (j in 1:fatherUniques){
    ...
    if(planted == 0){
      visibleOutputDebug <- "NA"
    }
    else if(planted < 40 & !(planted == 0)){
      visibleOutputDebug <- paste(psaDeath, planted, "STS", lifeRate, sep = " / ")
    }
    else{
      visibleOutputDebug <- paste(psaDeath, planted, lifeRate, sep = " / ")
    }
    print(visibleOutputDebug)
    if(fatherUniques == 1){
      endTablePrintout <- paste(visibleOutputDebug, sep = ",")
    }
    else{
      endTablePrintout <- paste(endTablePrintout, paste(visibleOutputDebug), sep = ", ")
    }
  }
}
...
print(endTablePrintout)

table = matrix(c(endTablePrintout), nrow=fatherLength-1, ncol=motherLength-1)
noquote(table)
```